...
read -p "[SERVER] - Informe o HOST local/NODE NAME deste servidor: [${DK_SERVER_HOST}] " -e answer
...
if [ "$DK_INSTALL_TYPE" = "M" ]; then
	...
	read -p "[REPO] - Utilizar Backup DB com AWS S3? (Y/n) [${DK_BK_INST}] " -e answer
	DK_BK_INST=${answer:-${DK_BK_INST}}
